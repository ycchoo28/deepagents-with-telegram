...
def _detect_provider(model_name: str) -> str | None:
    ...
    First checks for explicit OPENAI_PROVIDER environment variable override,
    then falls back to pattern matching on model name.
    ...
    """
    # Check for explicit provider override (useful for custom OpenAI-compatible APIs)
    provider_override = os.environ.get("OPENAI_PROVIDER", "").lower()
    if provider_override in ("openai", "anthropic", "google"):
